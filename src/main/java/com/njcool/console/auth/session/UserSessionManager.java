...
import com.njcool.console.auth.domain.UserDomain;
import com.njcool.console.auth.domain.UserOnlineDomain;
...
public class UserSessionManager {
     ...
     * session status
     */
    public static final String SESSION_STATUS ="biz-online-status";
     ...
     * @return
     */
    public List<UserOnlineDomain> getAllUser() {
        //获取所有session
        Collection<Session> sessions = userSessionDAO.getActiveSessions();
        List<UserOnlineDomain> list = new ArrayList<UserOnlineDomain>();

        for (Session session : sessions) {
            UserOnlineDomain bo = getSessionBo(session);
            if(null != bo) {
                list.add(bo);
            }
        }
    ...
    public List<SimplePrincipalCollection> getSimplePrincipalCollectionByUserId(Long...userIds) {
        ...
        Set<Long> idset = new HashSet<Long>(new ArrayList<Long>(Arrays.asList(userIds)));
        //获取所有session
        Collection<Session> sessions = userSessionDAO.getActiveSessions();
        //定义返回
     ...
     * @return
     */
    public UserOnlineDomain getSession(String sessionId) {
        Session session = userSessionRepository.getSession(sessionId);
        return getSessionBo(session);
    }

    private UserOnlineDomain getSessionBo(Session session) {
        //获取session登录信息。
        Object obj = session.getAttribute(DefaultSubjectContext.PRINCIPALS_SESSION_KEY);
        if(null == obj) {
            return null;
        ...
        if(obj instanceof SimplePrincipalCollection) {
            ...
            if(null != obj && obj instanceof UserDomain) {
                //存储session + user 综合信息
                UserOnlineDomain userBo = new UserOnlineDomain((UserDomain)obj);
                //最后一次和系统交互的时间
                ...
                //session最后一次与系统交互的时间
                userBo.setLoginTime(session.getLastAccessTime());
                //回话到期 ttl(ms)
    ...
    public Map<String, Object> changeSessionStatus(Boolean status,String sessionIds) {
        ...
        try {
            ...
            for (String id : sessionIdArray) {
                Session session = userSessionRepository.getSession(id);
            ...
            LOG.error(String.format("改变Session状态错误，sessionId[%s]", sessionIds), e);
            map.put("status", 500);
    ...
    public void forbidUserById(Long id, Long status) {
        //获取所有在线用户
        for(UserOnlineDomain bo : getAllUser()) {
            Integer userId = bo.getId();
            //匹配用户ID
            if(userId.equals(id)) {
                //获取用户Session
                Session session = userSessionRepository.getSession(bo.getSessionId());
                //标记用户Session
                SessionStatus sessionStatus = (SessionStatus) session.getAttribute(SESSION_STATUS);
